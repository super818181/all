var httpModule = {
    loadMoreScoreList: function () {
        ...
        Vue.http.post(globalData.baseUrl + 'q/roomStatusList', data).then(function(response) {
            ...
            if (bodyData.result == 0) {
                ...
                for (var i = 0; i < bodyData.data.length;i++) {
                    ...
                    appData.gameScoreList.push({"number":item.room_number,"time":item.start_time,"status":item.status,"game_type": item.game_type});
                }
...
for (var i = 0; i < globalData.gameList.length; i++) {
    appData.gameItems.push({
        "avatar":globalData.fileUrl+globalData.gameList[i].game_icon,
...
var methods = {
    ...
    clickScoreItem: function (item) {
        ...
        var url = Htmls.getReviewUrl(item.game_type, item.number);
