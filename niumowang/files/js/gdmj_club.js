...
var controlMethod = {
    ...
    showRecord: function () {
        socketModule.sendHistoryScoreboard();
    },
...
var operationMethod = {
    click: function (num1, num2) {
        ...
        if (num1 == 0) {
            ...
            socketModule.sendChooseCard(num2);
            controlMethod.m4aAudioPlay(num2 % 100);
            appData.player[0].is_operation = true;
        } else if (num1 == 4) {
            if (num2 == 5) {
                socketModule.sendQiangGangHu(0);
            } else {
                socketModule.sendPassCard();
            }
            console.log("过");
            appData.player[0].is_operation = true;
        } else if (num1 == 5) {
            console.log("碰");
            socketModule.sendPengCard();
            appData.player[0].is_operation = true;
        } else if (num1 == 6) {
            console.log("杠");
            if (num2 == 1) {
                socketModule.sendAnGang();
            } else if (num2 == 2) {
                socketModule.sendJiaGang();
            } else if (num2 == 3) {
                socketModule.sendBaoGang();
            }

            appData.player[0].is_operation = true;
        } else if (num1 == 7) {
            console.log("胡");
            socketModule.sendHuCard();
            appData.player[0].is_operation = true;
        } else if (num1 == 8) {
            console.log("抢杠胡");
            socketModule.sendQiangGangHu(1, appData.game.qianggang_card);
            appData.player[0].is_operation = true;
        }
    },
    ...
    cardMove: function (num0) {
        ...
        if (num != 0) {
            ...
            $(".mine .cardNew").animate({"right": "12%"}, 400, function () {
                operationMethod.insertCard(num);
            });
            /*	if(num0==0)
    ...
    gang: function (num, type, card) {
        ...
        if (type == 3) {
            ...
            appData.player[num].pengGang.push({
                "card": card,
                "num": appData.player[num].pengGang.length,
                "step": 1,
                "type": 3
            });
            if (num == 0) {
    ...
    pengMove: function (to, num1, from, num2, card) {
        ...
        setTimeout(function () {
            ...
            if (to == 0) {
                ...
                $(".mine .cardReturn").eq(num1).animate({
                    "width": "5.5%",
                    "margin-top": (82 - appData.fromY) / 1.6267 + "%",
                    "margin-left": 24 - appData.fromX + (num1 * 36) + "%"
                }, 300)
            } else if (to == 1) {
                ...
                $(".others .player2 .cardReturn").eq(num1).animate({
                    "margin-top": (64 - appData.fromY) / 1.6267 - (num1 * 16.8) + "%",
                    "margin-left": 83 - appData.fromX + "%"
                }, 300)
            } else if (to == 2) {
                ...
                $(".others .player3 .cardReturn").eq(num1).animate({
                    "width": "3.3%",
                    "margin-top": (10.6 - appData.fromY) / 1.6267 + "%",
                    "margin-left": 75.7 - appData.fromX - (num1 * 22) + "%"
                }, 300)
            } else if (to == 3) {
                ...
                $(".others .player4 .cardReturn").eq(num1).animate({
                    "margin-top": (10 - appData.fromY) / 1.6267 + 10 + (num1 * 16.8) + "10%",
                    "margin-left": 12.5 - appData.fromX + "%"
                }, 300)
            }
        }, 400)
    },
...
var socketModule = {
    ...
    processGetScoreBoard: function (obj) {
        ...
        if (obj.data.bs == '') {
            ...
            receiveMethod.receiveLastScoreboard(obj.data.bs, obj.wh);
        }
    },
    gAI: function () {
        socketModule.sendData({
...
var receiveMethod = {
    receiveLastScoreboard: function (obj) {
        ...
        try {
            var data = new Date(parseInt(obj.t) * 1000);
    ...
    receivePrepareJoinRoom: function (obj) {
        if (obj.data.is_club) {
            if (obj.data.is_club == 1) {
                methods.clickShowAlert(7, '无法加入，请联系管理员');
                return;
            }
        }
        // socketModule.gAC();
        ...
        if (obj.data.room_status == 3) {
            ...
            //DynLoading.goto('gdmj' + globalData.maxCount, 'i=' + globalData.roomNumber);
        } else {
    ...
    receiveJoinRoom: function (obj) {
        ...
        if (appData.position.positionReady) {
            socketModule.sendUploadGeo(appData.position.longitude, appData.position.latitude);
    ...
    receiveWin: function (obj) {
        ...
        if (obj.data.winner_id == -1 || obj.data.horse_cards.length == 0) {
            ...
            for (h = 0; h < obj.data.horse_cards.length; h++) {
                appData.game.ma.push({
                    "card": Math.ceil(obj.data.horse_cards[h].card),
                    "isMa": obj.data.horse_cards[h].win,
                    "num": h
                })
            }
        }
...
var methods = {
    ...
    confirmAudioSetting: function (once) {
        ...
        console.log("musicOnce:", appData.musicOnce);
    ...
    wx.getLocation({
        ...
        success: function (res) {
            if (appData.connectOrNot && appData.game.room != 0) {
                socketModule.sendUploadGeo(res.longitude, res.latitude);
            } else {
